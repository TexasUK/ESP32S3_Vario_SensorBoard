...
public:
  ...
  // Sensor → Display
  inline void sendHelloSensor(){ 
    Serial.printf("[CSV] TX: HELLO,SENSOR\n");
    ser.println("HELLO,SENSOR"); 
  }
  inline void sendPong(){ 
    Serial.printf("[CSV] TX: PONG\n");
    ser.println("PONG"); 
  }
  ...
  inline void sendAck(const char* key, int32_t val){ 
    Serial.printf("[CSV] TX: ACK,%s,%ld\n", key, (long)val);
    ser.printf("ACK,%s,%ld\n", key, (long)val); 
  }
  inline void sendPolars(const char* polarList){ 
    Serial.printf("[CSV] TX: POLARS,%s\n", polarList);
    ser.printf("POLARS,%s\n", polarList); 
  }
  inline void sendPolarData(int index, const char* data){ 
    Serial.printf("[CSV] TX: POLAR_DATA,%d,%s\n", index, data);
    ser.printf("POLAR_DATA,%d,%s\n", index, data); 
  }
  inline void sendPolarDataChunk(const char* data){ 
    Serial.printf("[CSV] TX: POLAR_DATA_CHUNK,%s\n", data);
    ser.printf("POLAR_DATA_CHUNK,%s\n", data); 
  }
...
private:
  ...
  void handleLine(const char* l){
    ...
    // Special debug for GET_POLARS
    if (!strcmp(l, "GET_POLARS")) {
      Serial.println("[CSV] *** GET_POLARS COMMAND RECEIVED ***");
    }
    ...
    if (!strcmp(l,"GET_POLARS")) { if (onGetPolars) onGetPolars(); return; }
